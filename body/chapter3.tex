...
内存映射是由操作系统提供的一种将文件映射到某一进程的地址空间的一种技术。该技术的主要目的就是为了提升对磁盘数据的访问效率。通过内存映射技术，进程或者应用程序就可以对文件进行随机访问。在32位的操作系统上，内存映射的大小有限，最大只能映射2GB的文件；在64位的操作系统中，通过内存映射技术可以映射非常巨大的文件。文献\mycite{yuhuibin2010neicunyingshe,yangningxue2004neicunyingshe}中，对内存映射在大数据处理上进行相关研究，事实证明内存映射不仅有助于快速处理数据同时还能节省大量额外的IO管理等操作。所以，对于单机上的大规模图处理而言，内存映射技术可以很好的解决图处理过程中的随机访问的问题。
